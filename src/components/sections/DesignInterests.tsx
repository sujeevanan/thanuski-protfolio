        ...
        >
          {designInterests.map((interest, i) => (
            <motion.div
              ...
              className={`group relative glass rounded-2xl p-7 cursor-default overflow-hidden border border-white/80 hover:border-accent/30 transition-colors duration-300`}
            >
              {/* Gradient background */}
              <div
                className={`absolute inset-0 bg-gradient-to-br ${interest.gradient} opacity-60 rounded-2xl`}
              />

              {/* Hover glow */}
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-br from-accent/5 to-transparent rounded-2xl" />

              {/* Icon */}
              <motion.div
                whileHover={{ rotate: 15, scale: 1.15 }}
                transition={{ duration: 0.3 }}
                className="relative z-10 text-3xl mb-5 inline-block"
              >
                {interest.icon}
              </motion.div>
